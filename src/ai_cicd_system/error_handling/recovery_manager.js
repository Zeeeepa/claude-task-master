...
export class RecoveryManager {
	...
	async attemptRecovery(errorAnalysis, context = {}) {
		...
		try {
			...
			log(
				recoveryResult.success ? 'info' : 'warning',
	...
	async _executeRepair(strategy, errorAnalysis, context) {
		...
		try {
			...
			switch (classification.category) {
				...
				case ERROR_CATEGORIES.DEPENDENCY:
					repairResult = await this._repairDependencyError(
						errorAnalysis,
						context
					);
					break;
				...
				case ERROR_CATEGORIES.CONFIGURATION:
					repairResult = await this._repairConfigurationError(
						errorAnalysis,
						context
					);
					break;
				...
				case ERROR_CATEGORIES.VALIDATION:
					repairResult = await this._repairValidationError(
						errorAnalysis,
						context
					);
					break;
				...
				default:
					throw new Error(
						`No repair strategy for category: ${classification.category}`
					);
			}
	...
	_pruneCheckpoints() {
		...
		if (this.checkpoints.size > maxCheckpoints) {
			...
			const toDelete = sortedCheckpoints.slice(
				0,
				this.checkpoints.size - maxCheckpoints
			);
