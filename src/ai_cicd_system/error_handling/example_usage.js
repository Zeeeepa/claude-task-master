...
export class ErrorHandlingExamples {
	...
	async demonstrateErrorCategories() {
		...
		const errorTypes = [
			...
			{
				name: 'Timeout Error',
				error: Object.assign(new Error('Request timeout'), {
					code: 'ETIMEDOUT'
				}),
	...
	async demonstrateContextPreservation() {
		...
		if (result.contextId) {
			const preservedContext = await this.errorSystem.contextManager.getContext(
				result.contextId
			);
			console.log(
				'Preserved context size:',
				JSON.stringify(preservedContext.data).length
			);
		}
	}
	...
	async demonstrateCircuitBreaker() {
		...
		for (let i = 1; i <= 8; i++) {
			try {
				await this.errorSystem.executeWithErrorHandling(failingOperation, {
					maxRetries: 1,
				...
				if (circuitStats) {
					...
					if (circuitStats.state === 'OPEN') {
						console.log(
							'Circuit breaker opened! Subsequent calls will be rejected.'
						);
						break;
					}
				}
	...
	async demonstrateCustomWorkflow() {
		...
		const complexOperation = async () => {
			...
			for (const step of steps) {
				...
				if (Math.random() < step.failureRate) {
					...
					error.code = step.name.includes('API')
						? 'NETWORK_ERROR'
						: 'PROCESSING_ERROR';
					throw error;
				}
		...
		try {
			...
			const activeEscalations =
				this.errorSystem.escalationEngine.getActiveEscalations();
	...
	async runAllExamples() {
		console.log(
			'🔄 Intelligent Error Handling & Auto-Recovery System Examples'
		);
		console.log(
			'================================================================'
		);
		...
		try {
			...
			console.log(
				...
				finalStats.overview.totalOperations
			);
			console.log(
				'Overall success rate:',
				Math.round(finalStats.overview.successRate * 100) + '%'
			);
