...
export class RetryStrategyManager {
	...
	async _executeWithStrategy(operation, strategy, retrySession) {
		...
		while (attempt <= strategy.maxRetries) {
			...
			try {
				...
				// Execute operation
				const result = await this._executeAttempt(
					operation,
					attempt,
					retrySession
				);
				...
				if (
					...
					!this._shouldRetry(error, strategy, retrySession)
				) {
	...
	_selectStrategy(config, context) {
		...
		if (this.config.enableAdaptive && context.errorCategory) {
			const adaptiveStrategy = this.adaptiveStrategies.get(
	...
	getStatistics() {
		...
		for (const session of this.retryHistory) {
			strategyStats[session.strategy] =
				(strategyStats[session.strategy] || 0) + 1;
			outcomeStats[session.state] = (outcomeStats[session.state] || 0) + 1;
		}

		const totalSessions = this.retryHistory.length;
		const successfulSessions = this.retryHistory.filter(
			(s) => s.state === RETRY_STATES.SUCCEEDED
		).length;
		...
		return {
			...
			outcomeStats,
			averageDuration:
				this.retryHistory.reduce((sum, s) => sum + (s.duration || 0), 0) /
					totalSessions || 0,
			averageAttempts:
				this.retryHistory.reduce((sum, s) => sum + s.attempts.length, 0) /
					totalSessions || 0,
