...
export class ContextManager {
	constructor(config = {}) {
		this.config = {
			...
			defaultStrategy:
				config.defaultStrategy || PRESERVATION_STRATEGIES.SELECTIVE,
	...
	async createContext(type, data, options = {}) {
		...
		try {
			...
			if (
				...
				context.metadata.processedSize > 1024
			) {
				...
				if (compressed.size < context.metadata.processedSize * 0.8) {
					...
					context.metadata.compressionRatio =
						compressed.size / context.metadata.processedSize;
				}
			}
	...
	async updateContext(contextId, updates, options = {}) {
		...
		try {
			...
			if (context.metadata.compressed) {
				...
				updatedData = this._mergeContextData(
					decompressed,
					updates,
	...
	_cleanupContexts() {
		...
		let totalSize = 0;
		const contextsByAge = Array.from(this.contexts.entries()).sort(
	...
	getStatistics() {
		...
		for (const context of this.contexts.values()) {
			...
			strategyStats[context.strategy] =
				(strategyStats[context.strategy] || 0) + 1;
			totalSize += context.metadata.processedSize;
		}
...
class ContextMetrics {
	...
	_updateUsagePatterns(context) {
		...
		if (context.data && typeof context.data === 'object') {
			...
			patterns.frequentlyAccessedFields = Array.from(
				patterns.fieldAccess.entries()
			)
				.sort((a, b) => b[1] - a[1])
				.slice(0, 10)
				.map(([field]) => field);
