...
export class DatabaseConnection {
	...
	async initialize() {
		...
		if (validation.warnings.length > 0) {
			log(
				'warn',
				`Database configuration warnings: ${validation.warnings.join(', ')}`
			);
		}
	...
	async _createPoolWithRetry() {
			...
			try {
				...
				if (attempt < maxAttempts) {
					const delay =
						...
						Math.pow(this.config.retry.backoff_factor, attempt - 1);
		...
		throw new Error(
			`Failed to connect to database after ${maxAttempts} attempts. Last error: ${lastError.message}`
		);
	}
	...
	async _testConnection() {
		...
		try {
			...
			log(
				'debug',
				`Database connection test successful. Server time: ${result.rows[0].current_time}`
			);
		} finally {
			client.release();
		}
	...
	async query(text, params = [], options = {}) {
		...
		try {
			if (this.config.monitoring.log_queries) {
				log(
					'debug',
					`Executing query ${queryId}: ${text.substring(0, 100)}${text.length > 100 ? '...' : ''}`
				);
			}
			...
			try {
				if (options.timeout || this.config.query_timeout) {
					client.query('SET statement_timeout = $1', [
						options.timeout || this.config.query_timeout
					]);
				}
				...
				if (executionTime > this.config.monitoring.slow_query_threshold_ms) {
					...
					if (this.config.monitoring.log_slow_queries) {
						log(
							'warn',
							`Slow query detected (${executionTime}ms): ${text.substring(0, 200)}${text.length > 200 ? '...' : ''}`
						);
					}
				}
	...
	_startHealthMonitoring() {
		...
		log(
			'debug',
			`Health monitoring started (interval: ${this.config.health_check.interval_ms}ms)`
		);
	}
