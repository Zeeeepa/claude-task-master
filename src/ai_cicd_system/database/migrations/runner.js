...
export class MigrationRunner {
	...
	async runMigrations() {
		...
		log(
			'info',
			`Successfully applied ${appliedMigrationsList.length} migrations`
		);
	...
	async rollbackLastMigration() {
		...
		try {
			...
			throw new Error(
				`Rollback script not found for migration ${lastMigration}`
			);
		}
		...
		for (const migration of migrationFiles) {
			try {
				...
				if (
					...
					warnings.push(
						`Migration may not contain any DDL statements: ${migration.filename}`
					);
				}
			} catch (error) {
