...
export class RateLimiter {
	...
	_calculateDelay() {
		...
		switch (this.config.backoffStrategy) {
			case 'linear':
				return Math.min(
					this.config.baseDelay * queueLength,
					this.config.maxDelay
				);

			case 'exponential':
				return Math.min(
					this.config.baseDelay * Math.pow(2, Math.min(queueLength, 10)),
					this.config.maxDelay
				);
	...
	getTimeUntilNextRequest() {
		...
		if (this.tokens <= 0) {
			const tokenWaitTime =
				this.config.burstRefillRate - (now - this.lastRefill);
...
export class QuotaManager {
	...
	recordUsage(count = 1) {
		...
		log(
			'debug',
			`Quota usage recorded: ${count} requests. Daily: ${this.usage.daily}/${this.config.dailyLimit}, Monthly: ${this.usage.monthly}/${this.config.monthlyLimit}`
		);
	}
	...
	checkQuota(requestCount = 1) {
		...
		const canProceed =
			dailyRemaining >= requestCount && monthlyRemaining >= requestCount;
	...
	_checkWarnings() {
		...
		for (const threshold of this.config.warningThresholds) {
			...
			if (dailyPercentage >= threshold && !this.warningsIssued.has(dailyKey)) {
				log(
					'warning',
					`Daily quota warning: ${(dailyPercentage * 100).toFixed(1)}% used (${this.usage.daily}/${this.config.dailyLimit})`
				);
			...
			if (
				...
				!this.warningsIssued.has(monthlyKey)
			) {
				log(
					'warning',
					`Monthly quota warning: ${(monthlyPercentage * 100).toFixed(1)}% used (${this.usage.monthly}/${this.config.monthlyLimit})`
				);
