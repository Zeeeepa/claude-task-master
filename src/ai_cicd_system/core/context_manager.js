...
export class ContextManager {
	...
	async generatePromptContext(taskId, options = {}) {
		...
		try {
			const context = await this.promptContextGenerator.generateContext(
				taskId,
				options
			);
			...
			log(
				'error',
				`Failed to generate prompt context for task ${taskId}: ${error.message}`
			);
			throw error;
		}
	}
...
class PromptContextGenerator {
	...
	async _generateCodebaseContext(taskId) {
		return {
			...
			dependencies: ['express@4.18.0', 'mongoose@6.0.0', 'jest@28.0.0'],
			file_structure: {
