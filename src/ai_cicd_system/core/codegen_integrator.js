...
export class CodegenIntegrator {
	constructor(config = {}) {
		...
		log(
			'info',
			`Codegen integrator initialized in ${this.config.get('mode')} mode`
		);
	}
	...
	async _validateConnection() {
		...
		try {
			...
			throw new CodegenError(
				...
				`Failed to validate Codegen API connection: ${error.message}`,
				error
			);
		}
	}
	...
	async processTask(task, taskContext) {
		...
		log(
			'info',
			`Processing task ${task.id} with codegen (request: ${requestId})`
		);
		...
		try {
			...
			const result = {
				...
				metrics: {
					...
					processing_time_ms:
						Date.now() -
						this.activeRequests.get(requestId).started_at.getTime(),
			...
			log(
				'error',
				`Failed to process task ${task.id}: ${handlingResult.error.message}`
			);
...
class PromptGenerator {
	...
	_buildPromptContent(task, context, template) {
		...
		content = content.replace(
			'{{REQUIREMENTS}}',
			this._formatRequirements(task.requirements)
		);
		content = content.replace(
			'{{ACCEPTANCE_CRITERIA}}',
			this._formatAcceptanceCriteria(task.acceptanceCriteria)
		);
		content = content.replace(
			'{{AFFECTED_FILES}}',
...
class PRTracker {
	...
	async getPRStatistics() {
		...
		return {
			...
			success_rate:
				prs.length > 0
					? (prs.filter((pr) => pr.status === 'merged').length / prs.length) *
						100
					: 0
		};
	}
		...
		try {
			...
			if (error.response) {
				...
				if (status === 401 || status === 403) {
					...
					errorMessage =
						'API server error. The service may be experiencing issues.';
				}
			} else if (
				...
				errorType = 'timeout';
				errorMessage =
					'API request timed out. The service may be experiencing high load.';
			}
