...
export class ComponentRegistry {
	...
	register(name, component, config = {}) {
		...
		log(
			'debug',
			`Component registered: ${name} with ${componentMetadata.dependencies.length} dependencies`
		);
	}
		...
		const visit = (name) => {
			...
			for (const dep of deps) {
				if (!this.components.has(dep)) {
					throw new Error(
						`Component '${name}' depends on unregistered component '${dep}'`
					);
				}
				visit(dep);
			}
	...
	async _runSingleHealthCheck(name, healthCheck) {
		try {
			...
			log(
				'warning',
				`Health check failed for component ${name}: ${error.message}`
			);
