...
export class LifecycleManager {
	...
	async initializeAll(options = {}) {
		...
		try {
			...
			if (parallel) {
				await this._initializeInParallel(
					sortedComponents,
					timeout,
					continueOnError,
					results
				...
				await this._initializeSequentially(
					sortedComponents,
					timeout,
					continueOnError,
					results
			...
			log(
				'info',
				`Component initialization completed: ${results.successful} successful, ${results.failed} failed, ${results.skipped} skipped`
			);
			return results;
		} catch (error) {
	...
	async _initializeSequentially(
		...
		for (const componentName of sortedComponents) {
			try {
				const timing = await this._initializeComponentWithTimeout(
					componentName,
					timeout
				);
				results.successful++;
	...
	_groupByDependencyLevel(sortedComponents) {
		...
		while (processed.size < sortedComponents.length) {
			...
			for (const componentName of sortedComponents) {
				...
				const dependencies =
					this.registry.dependencies.get(componentName) || [];
				const allDepsProcessed = dependencies.every((dep) =>
					processed.has(dep)
				);
		...
		try {
			await Promise.race([
				...
				new Promise((_, reject) =>
					setTimeout(
						() =>
							reject(new Error(`Component initialization timeout: ${name}`)),
						timeout
					)
				)
			]);
			...
			const duration = Date.now() - startTime;
			throw new Error(
				`Failed to initialize component '${name}': ${error.message}`
			);
		}
	}
	...
	async initializeComponent(name) {
		...
		for (const dep of dependencies) {
			...
			if (!depMetadata || depMetadata.status !== 'initialized') {
				throw new Error(
					`Dependency '${dep}' of component '${name}' is not initialized`
				);
			}
		}
		...
		try {
			...
			log(
				'error',
				`❌ Component initialization failed: ${name} - ${error.message}`
			);
			throw error;
		} finally {
	...
	async _performComponentInitialization(name, metadata) {
		...
		try {
			if (typeof component.initialize === 'function') {
				...
				throw new Error(
					`Component '${name}' does not have an initialize method`
				);
			}
		} catch (error) {
			throw new Error(
				`Component '${name}' initialization failed: ${error.message}`
			);
		}
	}
	...
	async shutdownAll(options = {}) {
		...
		for (const componentName of this.shutdownOrder) {
			try {
				const timing = await this._shutdownComponentWithTimeout(
					componentName,
					timeout
				);
				results.successful++;
		...
		log(
			'info',
			`Component shutdown completed: ${results.successful} successful, ${results.failed} failed`
		);
		return results;
	}
	...
	async _shutdownComponentWithTimeout(name, timeout) {
		...
		try {
			await Promise.race([
				...
				new Promise((_, reject) =>
					setTimeout(
						() => reject(new Error(`Component shutdown timeout: ${name}`)),
						timeout
					)
				)
			]);
			...
			throw new Error(
				`Failed to shutdown component '${name}': ${error.message}`
			);
		}
	}
	...
	async shutdownComponent(name) {
		...
		if (!metadata) {
			log(
				'warning',
				`Component '${name}' not found in registry during shutdown`
			);
			return;
		}
