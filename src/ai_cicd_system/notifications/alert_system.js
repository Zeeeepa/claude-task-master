...
export class AlertSystem {
    constructor(config = {}) {
        this.config = {
            ...
            defaultChannels: config.defaultChannels || [NOTIFICATION_CHANNELS.CONSOLE],
            rateLimiting: {
            ...
            escalationRules: config.escalationRules || this._getDefaultEscalationRules(),
    ...
    async sendAlert(alertData, options = {}) {
        ...
        try {
            // Check rate limiting
            if (this.config.rateLimiting.enabled && !this.rateLimiter.allowAlert(alert)) {
                return { 
                    success: false, 
            ...
            alert.status = deliveryResults.some(r => r.success) ? 
                ALERT_STATUSES.SENT : ALERT_STATUSES.FAILED;
    ...
    async sendRecoveryAlert(recoveryResult, options = {}) {
        const alertData = {
            ...
            priority: recoveryResult.success ? ALERT_PRIORITIES.MEDIUM : ALERT_PRIORITIES.HIGH,
            data: {
    ...
    _initializeChannels() {
        ...
        this.channels.set(NOTIFICATION_CHANNELS.EMAIL, new EmailChannel(this.config.email));
        this.channels.set(NOTIFICATION_CHANNELS.SLACK, new SlackChannel(this.config.slack));
        this.channels.set(NOTIFICATION_CHANNELS.SMS, new SMSChannel(this.config.sms));
        this.channels.set(NOTIFICATION_CHANNELS.WEBHOOK, new WebhookChannel(this.config.webhook));
    }
    ...
    _getDefaultEscalationRules() {
        return {
            [ALERT_PRIORITIES.EMERGENCY]: {
                channels: [NOTIFICATION_CHANNELS.SMS, NOTIFICATION_CHANNELS.SLACK, NOTIFICATION_CHANNELS.EMAIL],
    ...
    getStatistics() {
        ...
        for (const alert of this.alertHistory) {
            ...
            for (const result of alert.deliveryResults || []) {
                channelStats[result.channel] = channelStats[result.channel] || { sent: 0, failed: 0 };
                if (result.success) {
        ...
        const successfulAlerts = this.alertHistory.filter(a => a.status === ALERT_STATUSES.SENT).length;
    ...
    getActiveAlerts() {
        return Array.from(this.alerts.values())
            .filter(alert => alert.status !== ALERT_STATUSES.RESOLVED);
    }
...
class RateLimiter {
    ...
    getStatistics() {
        const now = Date.now();
        return {
            alertsInLastMinute: this.minuteWindow.filter(time => now - time < 60000).length,
            alertsInLastHour: this.hourWindow.filter(time => now - time < 3600000).length,
