...
async function basicRequirementProcessing() {
	...
	try {
		...
		result.tasks.forEach((task, index) => {
			console.log(
				`  ${index + 1}. ${task.title} (complexity: ${task.complexityScore}/10)`
			);
		});
		...
		result.codegen_results.forEach((pr, index) => {
			if (pr.pr_info) {
				console.log(
					`  ${index + 1}. ${pr.pr_info.title} - ${pr.pr_info.pr_url}`
				);
			}
		});
	...
	try {
		...
		for (let i = 0; i < requirements.length; i++) {
			...
			try {
				...
				// Get workflow status
				const workflowStatus = await system.getWorkflowStatus(
					result.workflow_id
				);
				console.log(
					`📊 Workflow progress: ${workflowStatus.progress?.toFixed(1) || 0}%`
				);
			} catch (error) {
...
async function componentLevelUsage() {
	...
	try {
		...
		console.log(
			`   📊 Average complexity: ${analysisResult.summary.averageComplexity.toFixed(1)}`
		);
		...
		console.log(
			`   📝 Context includes: ${Object.keys(promptContext).join(', ')}`
		);
		...
		if (codegenResult.pr_info) {
			...
			console.log(
				`   📊 Overall score: ${validationResult.score.overall_score} (${validationResult.score.grade})`
			);
...
async function errorHandlingExample() {
	...
	try {
		...
		try {
			...
			console.log(
				`   ✅ Correctly handled empty requirement: ${error.message}`
			);
		}
		...
		try {
			...
			console.log(
				`   ✅ Processed complex requirement: ${result.tasks.length} tasks generated`
			);
		} catch (error) {
...
async function runAllExamples() {
	...
	console.log(
		'This demo showcases the comprehensive AI-CICD system capabilities'
	);
