...
class PerformanceTracker {
	...
	async getAnalytics(options = {}) {
		...
		for (const [name, metric] of this.metrics) {
			...
			analytics.metrics_by_type[type] =
				(analytics.metrics_by_type[type] || 0) + 1;
		}
	...
	async getStatistics() {
		return {
			...
			total_data_points: Array.from(this.timeSeries.values()).reduce(
	...
	async _evaluateRule(ruleName, rule, data) {
		try {
			...
			switch (rule.type) {
				case 'health':
					shouldAlert =
						data.status === 'unhealthy' || data.status === 'degraded';
					break;
				case 'event':
