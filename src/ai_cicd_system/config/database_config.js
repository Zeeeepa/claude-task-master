...
export const dbConfig = {
	...
	pool: {
		...
		acquireTimeoutMillis:
			parseInt(process.env.DB_POOL_ACQUIRE_TIMEOUT) || 30000,
		...
		createRetryIntervalMillis:
			parseInt(process.env.DB_POOL_CREATE_RETRY_INTERVAL) || 200
	},
	...
	monitoring: {
		slow_query_threshold_ms:
			parseInt(process.env.DB_SLOW_QUERY_THRESHOLD) || 1000,
...
export function validateConfig() {
	...
	if (dbConfig.pool.max > 100)
		warnings.push('DB_POOL_MAX is very high (>100), consider reducing');

	// Timeout validation
	if (dbConfig.query_timeout < 1000)
		warnings.push('DB_QUERY_TIMEOUT is very low (<1s)');
	if (dbConfig.query_timeout > 300000)
		warnings.push('DB_QUERY_TIMEOUT is very high (>5min)');
