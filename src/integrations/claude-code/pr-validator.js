...
export class PRValidator {
	constructor(options = {}) {
		...
		this.qualityGates =
			options.qualityGates || this.client.config.getQualityGates();
	...
	async validatePR(prInfo, options = {}) {
		...
		try {
			...
			const recommendation =
				this.makeApprovalRecommendation(qualityGateResults);
...
Provide scores (0-1) and specific recommendations.`;
		...
		const result = await this.client.query(query, {
			systemPrompt:
				'You are a senior code reviewer. Provide detailed quality assessment with actionable feedback.'
		});
...
Provide performance scores and optimization recommendations.`;
		...
		const result = await this.client.query(query, {
			systemPrompt:
				'You are a performance optimization expert. Focus on measurable improvements.'
		});
	...
	async analyzeTestCoverage(changes) {
		const testFiles = changes.changedFiles.filter(
			(file) =>
...
Provide coverage score and recommendations for additional tests.`;
		...
		const result = await this.client.query(query, {
			systemPrompt:
				'You are a testing expert. Focus on comprehensive test coverage and quality.'
		});
...
Provide documentation score and specific improvement suggestions.`;
		...
		const result = await this.client.query(query, {
			systemPrompt:
				'You are a documentation expert. Focus on clarity and completeness.'
		});
...
Classify severity and provide migration guidance.`;
		...
		const result = await this.client.query(query, {
			systemPrompt:
				'You are an API compatibility expert. Focus on identifying breaking changes and migration paths.'
		});
	...
	async applyQualityGates(analysisResults) {
		...
		const criticalVulns = securityVulns.filter(
	...
	async generateValidationReport(data) {
		const { prInfo, qualityGateResults, analysisResults, validationTime } =
			data;
...
## Quality Gates
${Object.entries(qualityGateResults.gates)
	.map(
		([gate, result]) =>
