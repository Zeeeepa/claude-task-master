...
export class CodeAnalyzer {
    constructor(options = {}) {
        ...
        this.fileExtensions = options.fileExtensions || ['.js', '.ts', '.jsx', '.tsx', '.py', '.java', '.cpp', '.c', '.cs'];
        this.excludePatterns = options.excludePatterns || ['node_modules', '.git', 'dist', 'build', 'coverage'];
    ...
    async analyzeCodeQuality(target, options = {}) {
        ...
        try {
            ...
            const recommendations = this.generateRecommendations(fileAnalyses, projectAnalysis, metrics);
    ...
    async analyzeFiles(files, options = {}) {
        ...
        for (let i = 0; i < files.length; i += this.batchSize) {
            ...
            try {
                ...
                console.log(`Analyzed batch ${Math.floor(i / this.batchSize) + 1}/${Math.ceil(files.length / this.batchSize)}`);
            } catch (error) {
    ...
    async analyzeFile(filePath, options = {}) {
        try {
            ...
            return {
                file: relativePath,
                success: true,
                analysis: this.parseFileAnalysis(result.data?.lastResponse || result.data),
                size: content.length,
                lines: content.split('\n').length
            };
...
Provide specific, actionable feedback with clear scores and recommendations.`;
    ...
    parseFileAnalysis(response) {
        ...
        try {
            // Extract score
            const scoreMatch = response.match(/(?:score|quality)[:\s]*([0-9.]+)(?:\/10)?/i);
            if (scoreMatch) {
            ...
            // Generate summary
            analysis.summary = response.substring(0, 300) + (response.length > 300 ? '...' : '');
...
Provide scores and specific recommendations for improvement.`;
            ...
            const result = await this.client.query(query, {
                systemPrompt: 'You are a software architect analyzing project-level code quality and structure.',
            ...
            return this.parseProjectAnalysis(result.data?.lastResponse || result.data);
    ...
    calculateMetrics(fileAnalyses, projectAnalysis) {
        ...
        const scores = successfulAnalyses.map(a => a.analysis.score).filter(s => s > 0);
        const averageFileScore = scores.length > 0 ? 
        ...
        const issueCount = successfulAnalyses.reduce((sum, a) => sum + (a.analysis.issues?.length || 0), 0);
        const recommendationCount = successfulAnalyses.reduce((sum, a) => sum + (a.analysis.recommendations?.length || 0), 0);
        
        // Calculate overall score (weighted average of file scores and project factors)
        const overallScore = averageFileScore * 0.7 + (projectAnalysis.success ? 0.3 : 0);
    ...
    generateRecommendations(fileAnalyses, projectAnalysis, metrics) {
        ...
        if (metrics.overallScore < 0.6) {
            recommendations.priority.push('Overall code quality is below acceptable threshold - immediate attention required');
        }
        
        if (metrics.issueCount > fileAnalyses.length * 2) {
            recommendations.priority.push('High number of issues detected - consider code review and refactoring');
        }
        
        // General recommendations
        if (metrics.averageFileScore < 0.7) {
            recommendations.general.push('Improve code quality standards and review processes');
        }
        
        if (metrics.analysisSuccessRate < 0.9) {
            recommendations.general.push('Some files could not be analyzed - check for syntax errors or unsupported formats');
        }
        ...
        if (projectAnalysis.recommendations) {
            recommendations.projectLevel = projectAnalysis.recommendations.slice(0, 5);
        }
...
${recommendations.fileSpecific.map(item => 
...
<body>
    ...
    <div class="section">
        ...
        ${results.recommendations.fileSpecific.map(item => 
