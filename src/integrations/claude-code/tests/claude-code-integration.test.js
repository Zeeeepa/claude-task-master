...
import { ClaudeCodeIntegration, ClaudeCodeClient, PRValidator, CodeAnalyzer, FeedbackProcessor } from '../index.js';
...
describe('Claude Code Integration', () => {
                    ...
                    stdout: { on: jest.fn((event, callback) => {
                        if (command === 'git') {
                            ...
                            callback(JSON.stringify({
                                role: 'assistant',
                                content: 'Code quality analysis complete. Score: 8.5/10. No critical issues found.',
                                type: 'message'
                            }));
                        }
                    }) },
    ...
    describe('Code Quality Analysis', () => {
        ...
        test('should analyze code quality successfully', async () => {
            ...
            const fs = require('fs');
            fs.statSync.mockReturnValue({ isFile: () => true, isDirectory: () => false, size: 1000 });
                ...
                const mockProcess = {
                    stdout: { on: jest.fn((event, callback) => {
                        if (command === 'claude') {
                            callback(JSON.stringify({
                                role: 'assistant',
                                content: 'Code quality score: 7.5/10. Issues: Missing error handling. Recommendations: Add try-catch blocks.',
                                type: 'message'
                            }));
                        }
                    }) },
                ...
                const mockProcess = {
                    stdout: { on: jest.fn((event, callback) => {
                        if (command === 'claude') {
                            callback(JSON.stringify({
                                role: 'assistant',
                                content: 'Project analysis complete. Overall score: 8.0/10.',
                                type: 'message'
                            }));
                        }
                    }) },
                ...
                const mockProcess = {
                    stdout: { on: jest.fn((event, callback) => {
                        if (command === 'claude') {
                            callback(JSON.stringify({
                                role: 'assistant',
                                content: 'Root cause: Missing dependency. Solution: Run npm install missing-package.',
                                type: 'message'
                            }));
                        }
                    }) },
                ...
                const mockProcess = {
                    stdout: { on: jest.fn((event, callback) => {
                        if (command === 'claude') {
                            callback(JSON.stringify({
                                role: 'assistant',
                                content: 'Security scan complete. Found 2 medium severity issues: SQL injection risk, XSS vulnerability.',
                                type: 'message'
                            }));
                        }
                    }) },
                ...
                const mockProcess = {
                    stdout: { on: jest.fn((event, callback) => {
                        if (command === 'claude') {
                            callback(JSON.stringify({
                                role: 'assistant',
                                content: 'Performance analysis complete. Bottleneck detected in database queries. Recommendation: Add indexing.',
                                type: 'message'
                            }));
                        }
                    }) },
    ...
    describe('Error Handling', () => {
        test('should throw error when not initialized', async () => {
            ...
            await expect(uninitializedIntegration.validatePullRequest({}))
                .rejects.toThrow('Claude Code integration not initialized');
                
            await expect(uninitializedIntegration.analyzeCodeQuality('./src'))
                .rejects.toThrow('Claude Code integration not initialized');
        });
        ...
        test('should handle timeout errors gracefully', async () => {
            ...
            const result = await integration.validatePullRequest(prInfo, { timeout: 100 });
            expect(result.success).toBe(false);
...
describe('Individual Components', () => {
    ...
    describe('FeedbackProcessor', () => {
        test('should generate GitHub feedback correctly', async () => {
            ...
            const result = await processor.processFeedback(analysisResults, { target: 'github' });
            
            expect(result.success).toBe(true);
            expect(result.feedback.formats.github).toBeDefined();
            expect(result.feedback.formats.github.content).toContain('Code Quality Analysis Report');
        ...
        test('should determine severity correctly', () => {
            ...
            expect(processor.determineSeverity('security vulnerability detected', 0.5)).toBe('critical');
            expect(processor.determineSeverity('missing error handling', 0.4)).toBe('high');
...
describe('End-to-End Scenarios', () => {
    test('should handle complete PR validation workflow', async () => {
        ...
        spawn.mockImplementation(() => ({
            stdout: { on: jest.fn((event, cb) => cb('{"role": "assistant", "content": "Analysis complete"}')) },
            stderr: { on: jest.fn() },
            stdin: { write: jest.fn(), end: jest.fn() },
