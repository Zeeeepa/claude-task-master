...
export class FeedbackProcessor {
    ...
    async processFeedback(analysisResults, options = {}) {
        ...
        try {
            ...
            if (target === 'all' || target.includes('github')) {
                feedback.formats.github = await this.generateGitHubFeedback(processedData, options);
            }
            
            if (target === 'all' || target.includes('linear')) {
                feedback.formats.linear = await this.generateLinearFeedback(processedData, options);
            }
            
            if (target === 'all' || target.includes('slack')) {
                feedback.formats.slack = await this.generateSlackFeedback(processedData, options);
            }
            
            if (target === 'all' || target.includes('email')) {
                feedback.formats.email = await this.generateEmailFeedback(processedData, options);
            }

            // Generate custom format if template provided
            if (customTemplate) {
                feedback.formats.custom = await this.generateCustomFeedback(processedData, customTemplate, options);
            }
    ...
    preprocessAnalysisData(analysisResults) {
        ...
        if (analysisResults.fileAnalyses) {
            for (const fileAnalysis of analysisResults.fileAnalyses) {
                if (fileAnalysis.success && fileAnalysis.analysis) {
                    ...
                    if (analysis.issues) {
                        data.issues.push(...analysis.issues.map(issue => ({
                    ...
                    if (analysis.recommendations) {
                        data.recommendations.push(...analysis.recommendations.map(rec => ({
                            file: fileAnalysis.file,
                            type: 'recommendation',
                            message: rec,
        ...
        if (analysisResults.recommendations) {
            if (analysisResults.recommendations.priority) {
                data.recommendations.push(...analysisResults.recommendations.priority.map(rec => ({
                    type: 'project',
                    message: rec,
                    priority: 'high'
                })));
            ...
            if (analysisResults.recommendations.general) {
                data.recommendations.push(...analysisResults.recommendations.general.map(rec => ({
                    type: 'project',
                    message: rec,
                    priority: 'medium'
                })));
        ...
        data.issues.sort((a, b) => this.getSeverityWeight(b.severity) - this.getSeverityWeight(a.severity));
    ...
    generateSummary(data) {
        ...
        if (data.metrics.overallScore !== undefined) {
            ...
            summary.overallStatus = summary.score >= 0.7 ? 'good' : summary.score >= 0.5 ? 'fair' : 'poor';
        }

        // Count critical issues and high priority recommendations
        summary.criticalIssues = data.issues.filter(issue => issue.severity === 'critical').length;
        summary.highPriorityRecommendations = data.recommendations.filter(rec => rec.priority === 'high').length;

        // PR-specific summary
        if (data.type === 'pr-validation') {
            summary.prStatus = data.recommendation?.approve ? 'approved' : 'changes-requested';
    ...
    async generateLinearFeedback(data, options = {}) {
        ...
        content += template.footer.replace('{{TIMESTAMP}}', new Date().toLocaleString());
    ...
    async generateEmailFeedback(data, options = {}) {
        ...
        // Add recommendations
        if (summary.highPriorityRecommendations > 0) {
            html += '<h3 style="color: #0366d6;">🎯 Priority Recommendations</h3><ul>';
            const highPriorityRecs = data.recommendations.filter(rec => rec.priority === 'high');
            for (const rec of highPriorityRecs.slice(0, 5)) {
        ...
        html += template.footer.replace('{{TIMESTAMP}}', new Date().toLocaleString());
    ...
    async generateCustomFeedback(data, template, options = {}) {
        ...
        if (content.includes('{{HIGH_PRIORITY_RECOMMENDATIONS}}')) {
            const highPriorityRecs = data.recommendations
                ...
                .map(rec => `- ${rec.message}`)
                .join('\n');
            content = content.replace(/\{\{HIGH_PRIORITY_RECOMMENDATIONS\}\}/g, highPriorityRecs);
        }
    ...
    determineSeverity(issue, score) {
        if (issue.toLowerCase().includes('security') || issue.toLowerCase().includes('vulnerability')) {
            return 'critical';
        }
