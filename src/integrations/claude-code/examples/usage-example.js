...
export async function basicPRValidation() {
	...
	try {
		...
		if (result.success) {
			...
			console.log(
				`Recommendation: ${result.validation.recommendation.approve ? 'APPROVE' : 'REQUEST CHANGES'}`
			);
			console.log(
				`Confidence: ${(result.validation.recommendation.confidence * 100).toFixed(1)}%`
			);
			console.log(
				`Validation time: ${(result.metrics.validationTime / 1000).toFixed(2)}s`
			);

			// Save feedback to file
			if (result.feedback?.formats?.github) {
				writeFileSync(
					'./pr-validation-feedback.md',
					result.feedback.formats.github.content
				);
...
export async function comprehensiveCodeAnalysis() {
	...
	try {
		...
		if (result.success) {
			...
			console.log(
				`Overall Score: ${(analysis.metrics.overallScore * 100).toFixed(1)}%`
			);
			...
			console.log(
				`Analysis Time: ${(metrics.analysisTime / 1000).toFixed(2)}s`
			);
			...
			if (feedback?.formats?.github) {
				writeFileSync(
					...
					feedback.formats.github.content
				);
				console.log('📄 Detailed report saved to code-analysis-report.md');
			}

			// Export results in multiple formats
			await integration.codeAnalyzer.exportResults(
				analysis,
				'json',
				'./code-analysis-results.json'
			);
			await integration.codeAnalyzer.exportResults(
				analysis,
				'html',
...
npm ERR! to accept an incorrect (and potentially broken) dependency resolution.
		...
		if (result.success) {
			...
			const debugSuggestions =
				result.debug.data?.lastResponse || result.debug.data;
			console.log(
				'Debug Suggestions:',
				debugSuggestions.substring(0, 300) + '...'
			);
			...
			if (result.feedback?.formats?.linear) {
				...
				console.log(
					'Content:',
					result.feedback.formats.linear.content.substring(0, 200) + '...'
				);
			}
		} else {
...
export async function performanceAnalysisExample() {
	...
	try {
		...
		if (result.success) {
			...
			const perfData =
				result.analysis.data?.lastResponse || result.analysis.data;
			...
			if (result.feedback?.formats?.email) {
				...
				writeFileSync(
					...
					result.feedback.formats.email.html
				);
	...
	try {
		...
		if (result.success) {
			...
			if (formats.slack) {
				writeFileSync(
					'./feedback-slack.json',
					JSON.stringify(formats.slack, null, 2)
				);
...
export async function metricsMonitoringExample() {
	...
	try {
		...
		console.log(
			...
			status.metrics.averageValidationTime + 'ms'
		);
		console.log(
			'Success Rate:',
...
Internal use only - Do not distribute
		...
		if (result.success) {
			...
			const customFeedback =
				await integration.feedbackProcessor.generateCustomFeedback(
