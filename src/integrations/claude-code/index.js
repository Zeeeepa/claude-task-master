...
export class ClaudeCodeIntegration {
	...
	async validatePullRequest(prInfo, options = {}) {
		...
		try {
			...
			const validationResult = await this.prValidator.validatePR(
				prInfo,
				options
			);
	...
	async analyzeCodeQuality(target, options = {}) {
		...
		try {
			...
			const analysisResult = await this.codeAnalyzer.analyzeCodeQuality(
				target,
				options
			);
	...
	async debugBuildFailure(errorLog, options = {}) {
		...
		try {
			...
			const debugResult = await this.client.debugBuildFailure(
				errorLog,
				options
			);
	...
	async analyzePerformance(target, options = {}) {
		...
		try {
			...
			const analysisResult = await this.client.analyzePerformance(
				target,
				options
			);
	...
	updateMetrics(operation, duration, success) {
		...
		const totalOperations =
			this.metrics.validationsPerformed + this.metrics.analysesCompleted;
		if (totalOperations > 0) {
			this.metrics.successRate =
				this.metrics.feedbackGenerated / totalOperations;
		}
	}
