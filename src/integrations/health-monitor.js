...
export class HealthMonitor extends EventEmitter {
	...
	getMetrics() {
		return {
			...
			failureRate:
				this.metrics.totalChecks > 0
					? (this.metrics.failedChecks / this.metrics.totalChecks) * 100
					: 0,
	...
	async _performHealthCheck(componentId) {
		...
		try {
			...
			if (healthCheck.options.retryAttempts > 0) {
				await this._retryHealthCheck(
					componentId,
					healthCheck.options.retryAttempts
				);
			}
		}
	...
	_checkAlert(componentId, status) {
		if (status.consecutiveFailures >= this.config.alertThreshold) {
			...
			if (alert && !alert.active) {
				...
				console.error(
					`🚨 Alert triggered for component ${componentId}: ${status.consecutiveFailures} consecutive failures`
				);
			}
		}
	}
