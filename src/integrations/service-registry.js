...
export class ServiceRegistry extends EventEmitter {
	...
	async unregister(serviceId) {
		...
		try {
			...
			console.log(
				`✅ Service ${service.name} (${serviceId}) unregistered successfully`
			);
		} catch (error) {
	...
	async _initializeStorage() {
		switch (this.config.storage) {
			...
			case 'consul':
				...
				console.log(
					'📡 Consul storage backend not yet implemented, using memory'
				);
				break;
			case 'etcd':
				// TODO: Initialize etcd client
				console.log(
					'📡 etcd storage backend not yet implemented, using memory'
				);
				break;
			default:
		...
		for (const [serviceId, heartbeatInfo] of this.heartbeats) {
			...
			if (timeSinceLastHeartbeat > heartbeatInfo.timeout) {
				...
				if (service && service.status === 'healthy') {
					...
					console.warn(
						`⚠️ Service ${service.name} (${serviceId}) marked as unhealthy`
					);
				}
			}
		}
