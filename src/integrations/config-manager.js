...
export class ConfigManager extends EventEmitter {
            ...
            for (const file of configFiles) {
                if (file.endsWith('.json') || file.endsWith('.js')) {
                    ...
                    try {
                        ...
                        console.warn(`⚠️ Failed to load config file ${file}:`, error.message);
                    }
                }
            }
    ...
    async _startFileWatching() {
        ...
        try {
            const watcher = watch(this.config.configDir, { recursive: true }, 
                (eventType, filename) => {
    ...
    async _saveConfiguration(configName, config, filePath = null) {
        try {
            const targetPath = filePath || 
                path.join(this.config.configDir, `${configName}.json`);
    ...
    async _validateConfiguration(configName, config) {
        ...
        try {
            ...
            throw new Error(`Configuration validation failed for ${configName}: ${error.message}`);
        }
    }
    ...
    async _backupConfiguration() {
        try {
            ...
            for (const [configName, config] of this.configurations) {
                const backupFile = path.join(backupDir, `${configName}-${timestamp}.json`);
                const content = JSON.stringify(config, null, 2);
