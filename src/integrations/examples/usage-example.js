...
async function runExample() {
    ...
    try {
        ...
        console.log('Found storage services:', Array.isArray(storageServices) ? 
            storageServices.map(s => s.name) : [storageServices.name]);

        const allServices = await framework.getAllComponents();
        console.log('All registered services:', allServices.map(s => s.name));
        console.log();
        ...
        for (const [componentId, health] of Object.entries(overallHealth.components)) {
