...
export class RateLimiter {
  ...
  _setupLimiters() {
    ...
    this.globalLimiter = rateLimit({
      ...
      legacyHeaders: false,
      keyGenerator: this.config.keyGenerator || this._defaultKeyGenerator.bind(this),
  ...
  _shouldSkipRequest(req) {
    ...
    if (this.config.skipSuccessfulRequests && req.res && req.res.statusCode < 400) {
      return true;
    }

    if (this.config.skipFailedRequests && req.res && req.res.statusCode >= 400) {
      return true;
    }
