...
export class DataTransformer {
	...
	_setupSchemas() {
		...
		// Compile schemas
		this.validateOrchestratorWorkflow = this.ajv.compile(
			this.orchestratorWorkflowSchema
		);
		this.validateClaudeCodeRequest = this.ajv.compile(
			this.claudeCodeRequestSchema
		);
		this.validateClaudeCodeResponse = this.ajv.compile(
			this.claudeCodeResponseSchema
		);
	}
	...
	async transformForClaudeCode(data) {
		try {
			this.logger.debug('Transforming data for Claude Code', {
			...
			if (this.config.validateOutput) {
				...
				if (!validation) {
					throw new Error(
						`Invalid Claude Code request format: ${this.ajv.errorsText()}`
					);
				}
			}
	...
	async transformFromClaudeCode(data) {
		try {
			this.logger.debug('Transforming data from Claude Code', {
				inputType: typeof data
			});
			...
			if (this.config.validateInput) {
				...
				if (!validation) {
					throw new Error(
						`Invalid Claude Code response format: ${this.ajv.errorsText()}`
					);
				}
			}

			const transformedData =
				await this._transformClaudeCodeToOrchestrator(data);
	...
	_isWorkflowCommand(data) {
		return data && typeof data === 'object' && data.workflowId && data.command;
	}
	...
	_isAnalysisRequest(data) {
		return (
			data && typeof data === 'object' && (data.analysisType || data.codebase)
		);
	}
	...
	async _transformAnalysisToClaudeCode(analysis) {
		return {
			...
			codebase: {
				...
				source:
					analysis.codebase?.repository || analysis.codebase?.source || '',
