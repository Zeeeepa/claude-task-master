...
export class AuthHandler {
	constructor(config = {}) {
		this.config = {
			jwtSecret:
				config.jwtSecret || process.env.JWT_SECRET || this._generateSecret(),
	...
	_generateSecret() {
		...
		this.logger.warn(
			'Using generated JWT secret. Set JWT_SECRET environment variable for production.'
		);
		return secret;
	}
	...
	generateRefreshToken(userId) {
		...
		const expiresAt = new Date(
			Date.now() + this._parseTimeToMs(this.config.refreshTokenExpiresIn)
		);
	...
	_recordFailedAttempt(identifier) {
		...
		this.logger.warn(
			`Failed login attempt for ${identifier}. Count: ${attempts.count}`
		);
	}
