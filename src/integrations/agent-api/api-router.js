...
export class APIRouter {
	...
	_setupOrchestratorRoutes() {
			...
			async (req, res, next) => {
				try {
					...
					const transformedPayload =
						await this.dataTransformer.transformForClaudeCode(payload);
					...
					const result = await this._processWorkflowCommand(
						workflowId,
						command,
						transformedPayload
					);
		...
		// Get workflow status
		this.router.get(
			'/orchestrator/workflow/:workflowId/status',
			this.authHandler.requireAuth(),
			async (req, res, next) => {
				try {
			...
			async (req, res, next) => {
				try {
					...
					const transformedRequest =
						await this.dataTransformer.transformForClaudeCode({
							codebase,
							analysisType,
							options
						});
					...
					const analysisResult = await this._forwardToClaudeCode(
						'analyze',
						transformedRequest
					);
					...
					const transformedResponse =
						await this.dataTransformer.transformFromClaudeCode(analysisResult);
		...
		this.router.post(
			...
			async (req, res, next) => {
				try {
					...
					const result = await this._forwardToClaudeCode('execute', {
						command,
						parameters
					});
		...
		this.router.post(
			...
			async (req, res, next) => {
				try {
					...
					const transformed =
						await this.dataTransformer.transformForClaudeCode(data);
		...
		this.router.post(
			...
			async (req, res, next) => {
				try {
					...
					const transformed =
						await this.dataTransformer.transformFromClaudeCode(data);
		...
		this.router.post(
			...
			async (req, res, next) => {
				try {
					...
					const validation = await this.dataTransformer.validateData(
						data,
						schema
					);
		...
		this.router.get(
			...
			async (req, res, next) => {
				try {
					...
					const errors = await this._getErrorLogs(
						parseInt(limit),
						parseInt(offset)
					);
	...
	async _processWorkflowCommand(workflowId, command, payload) {
		...
		this.logger.info(
			`Processing workflow command: ${command} for workflow: ${workflowId}`
		);
