...
describe('Performance Tests', () => {
    ...
    describe('Bulk Operations Performance', () => {
        test.skipIf(skipPerformanceTests)('should handle bulk task creation efficiently', async () => {
            ...
            console.log(`Created ${taskCount} tasks in ${totalTime}ms (${tasksPerSecond.toFixed(2)} tasks/second)`);
        ...
        test.skipIf(skipPerformanceTests)('should handle bulk queries efficiently', async () => {
            ...
            const queryTests = [
                ...
                { name: 'Filter by status', query: () => taskStorage.listTasks({ status: 'pending' }) },
                { name: 'Filter by type', query: () => taskStorage.listTasks({ type: 'bug' }) },
                { name: 'Filter by priority', query: () => taskStorage.listTasks({ priority: 8 }) },
                { name: 'Complex filter', query: () => taskStorage.listTasks({ 
                    type: 'feature', 
                    status: 'in_progress',
                    sort_by: 'priority',
                    sort_order: 'DESC'
                })},
                { name: 'Paginated query', query: () => taskStorage.listTasks({ limit: 50, offset: 100 }) },
            ...
            for (const test of queryTests) {
                ...
                console.log(`${test.name}: ${queryTime}ms (${Array.isArray(result) ? result.length : 1} results)`);
            }
        ...
        test.skipIf(skipPerformanceTests)('should handle bulk context operations efficiently', async () => {
            // Create a task first
            ...
            for (let i = 0; i < contextCount; i++) {
                const contextType = ['codebase', 'ai_interaction', 'validation', 'performance'][i % 4];
                const contextData = {
                    iteration: i,
                    timestamp: new Date(),
            ...
            console.log(`Created ${contextCount} contexts in ${totalTime}ms (${contextsPerSecond.toFixed(2)} contexts/second)`);
            ...
            console.log(`Retrieved ${contexts.length} contexts in ${retrievalTime}ms`);
    ...
    describe('Concurrent Operations Performance', () => {
        test.skipIf(skipPerformanceTests)('should handle concurrent task operations', async () => {
            const concurrentUsers = 20;
            const tasksPerUser = 10;
            const totalTasks = concurrentUsers * tasksPerUser;

            console.log(`Simulating ${concurrentUsers} concurrent users creating ${tasksPerUser} tasks each...`);

            const startTime = Date.now();

            // Simulate concurrent users
            const userPromises = Array.from({ length: concurrentUsers }, async (_, userIndex) => {
                const userTasks = Array.from({ length: tasksPerUser }, (_, taskIndex) => ({
            ...
            console.log(`Created ${totalTasks} tasks concurrently in ${totalTime}ms (${tasksPerSecond.toFixed(2)} tasks/second)`);
        ...
        test.skipIf(skipPerformanceTests)('should handle concurrent read/write operations', async () => {
            ...
            console.log(`Completed ${operations.length} concurrent operations in ${totalTime}ms (${operationsPerSecond.toFixed(2)} ops/second)`);
    ...
    describe('Memory and Resource Usage', () => {
        test.skipIf(skipPerformanceTests)('should maintain stable memory usage under load', async () => {
            ...
            for (let iteration = 0; iteration < iterations; iteration++) {
                ...
                const tasks = Array.from({ length: tasksPerIteration }, (_, i) => ({
                    ...
                    requirements: Array.from({ length: 10 }, (_, j) => `Requirement ${j}`),
                    acceptance_criteria: Array.from({ length: 5 }, (_, j) => `Criteria ${j}`),
                    metadata: {
                ...
                for (const taskId of taskIds) {
                    await taskStorage.storeTaskContext(taskId, 'performance', {
                        ...
                        iteration,
                        metrics: Array.from({ length: 20 }, (_, i) => ({ metric: i, value: Math.random() }))
                    });
                }
                ...
                await connection.query('DELETE FROM tasks WHERE title LIKE $1', [`Memory Test Task ${iteration}-%`]);
        ...
        test.skipIf(skipPerformanceTests)('should handle connection pool efficiently', async () => {
            ...
            console.log(`Completed ${concurrentOperations} operations in ${endTime - startTime}ms`);
    ...
    describe('Query Performance Optimization', () => {
        test.skipIf(skipPerformanceTests)('should demonstrate index effectiveness', async () => {
            ...
            const indexedQueries = [
                { name: 'Filter by status (indexed)', query: () => taskStorage.listTasks({ status: 'pending' }) },
                { name: 'Filter by priority (indexed)', query: () => taskStorage.listTasks({ priority: 8 }) },
                { name: 'Filter by assigned_to (indexed)', query: () => taskStorage.listTasks({ assigned_to: 'user-5' }) },
                { name: 'Filter by type (indexed)', query: () => taskStorage.listTasks({ type: 'bug' }) },
                { name: 'Sort by created_at (indexed)', query: () => taskStorage.listTasks({ 
                    sort_by: 'created_at', 
                    sort_order: 'DESC',
                    limit: 100 
                })},
                { name: 'Complex indexed query', query: () => taskStorage.listTasks({
                    status: 'in_progress',
                    type: 'feature',
                    sort_by: 'priority',
                    sort_order: 'DESC',
