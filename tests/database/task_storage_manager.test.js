...
describe('TaskStorageManager', () => {
    ...
    describe('Initialization', () => {
        ...
        test('should handle initialization errors gracefully', async () => {
            ...
            const { initializeDatabase } = await import('../../src/ai_cicd_system/database/connection.js');
    ...
    describe('Task Storage Operations', () => {
        ...
        test('should validate task data before storing', async () => {
            ...
            await expect(taskStorageManager.storeTask(invalidTask))
        ...
        test('should return null for non-existent task', async () => {
            const nonExistentTask = await taskStorageManager.getTask('non-existent-id');
        ...
        test('should update task status with context tracking', async () => {
            ...
            const statusChangeContext = context.find(c => c.context_type === 'status_change');
    ...
    describe('Task Listing and Filtering', () => {
        beforeEach(async () => {
            ...
            // Create test tasks
            const tasks = [
                { title: 'High Priority Task', priority: 9, status: 'pending', type: 'bug' },
                { title: 'Medium Priority Task', priority: 5, status: 'in_progress', type: 'feature' },
                { title: 'Low Priority Task', priority: 2, status: 'completed', type: 'feature' },
                { title: 'Another High Priority', priority: 8, status: 'pending', type: 'enhancement' }
            ];
        ...
        test('should support sorting', async () => {
            ...
            expect(tasksByPriority[0].priority).toBeGreaterThanOrEqual(tasksByPriority[1].priority);
        });
    });
    ...
    describe('Context Management', () => {
        ...
        test('should store task context', async () => {
            ...
            await taskStorageManager.storeTaskContext(taskId, 'test_context', contextData);

            const contexts = await taskStorageManager.getTaskContext(taskId);
            const testContext = contexts.find(c => c.context_type === 'test_context');
        ...
        test('should validate context data', async () => {
            await expect(taskStorageManager.storeTaskContext(taskId, 'invalid_type', {}))
        ...
        test('should store AI interaction context', async () => {
            ...
            await taskStorageManager.storeAIInteraction(taskId, 'claude-3', interactionData);
        ...
        test('should get full task context organized by type', async () => {
            ...
            await taskStorageManager.storeTaskContext(taskId, 'codebase', { files: ['test.js'] });
            await taskStorageManager.storeAIInteraction(taskId, 'gpt-4', { type: 'analysis' });
            await taskStorageManager.storeValidationResult(taskId, 'test', 'jest', 'passed', 100, {}, {});
    ...
    describe('Task Dependencies', () => {
        ...
        test('should store dependency context for both tasks', async () => {
            await taskStorageManager.addTaskDependency(parentTaskId, childTaskId, 'depends_on');

            const parentContext = await taskStorageManager.getTaskContext(parentTaskId);
            const childContext = await taskStorageManager.getTaskContext(childTaskId);

            const parentDependencyContext = parentContext.find(c => c.context_type === 'dependency_parent');
            const childDependencyContext = childContext.find(c => c.context_type === 'dependency_child');

            expect(parentDependencyContext).toBeDefined();
            expect(childDependencyContext).toBeDefined();
            expect(parentDependencyContext.context_data.child_task_id).toBe(childTaskId);
            expect(childDependencyContext.context_data.parent_task_id).toBe(parentTaskId);
        });
    });
    ...
    describe('Metrics and Analytics', () => {
        beforeEach(async () => {
            ...
            const tasks = [
                { title: 'Pending Task 1', status: 'pending', complexity_score: 5, estimated_hours: 2 },
                { title: 'Pending Task 2', status: 'pending', complexity_score: 7, estimated_hours: 4 },
                { title: 'In Progress Task', status: 'in_progress', complexity_score: 6, estimated_hours: 3 },
                { title: 'Completed Task 1', status: 'completed', complexity_score: 4, estimated_hours: 2, actual_hours: 2.5 },
                { title: 'Completed Task 2', status: 'completed', complexity_score: 8, estimated_hours: 5, actual_hours: 4.5 },
                { title: 'Failed Task', status: 'failed', complexity_score: 9, estimated_hours: 6 }
            ];
            ...
            for (const task of tasks) {
                ...
                if (task.actual_hours) {
                    await taskStorageManager.updateTask(taskId, { actual_hours: task.actual_hours });
                }
            }
        });
    ...
    describe('Error Handling', () => {
        ...
        test('should handle errors when task storage not initialized', async () => {
            const uninitializedStorage = new TaskStorageManager({ enable_mock: true });
            
            await expect(uninitializedStorage.storeTask({ title: 'Test' }))
        ...
        test('should track performance metrics on errors', async () => {
            ...
            try {
                await taskStorageManager.updateTask('non-existent-id', { title: 'Updated' });
            } catch (error) {
                // Expected to fail
            }
