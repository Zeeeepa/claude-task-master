...
describe('Database Models Integration Tests', () => {
    ...
    describe('Integration Tests', () => {
        test('should handle complete workflow with tasks and subtasks', async () => {
            ...
            const foundWorkflow = await workflowModel.findById(workflow.id);
            expect(foundWorkflow.task_count).toBe(1);

            const foundTask = await taskModel.findById(mainTask.id);
            expect(foundTask.subtask_count).toBe(2);

            const subtasks = await subtaskModel.findByParentId(mainTask.id);
            ...
            await subtaskModel.delete(subtask1.id);
            await subtaskModel.delete(subtask2.id);
            await taskModel.delete(mainTask.id);
            await workflowModel.delete(workflow.id);
        });
        ...
        test('should handle task dependencies correctly', async () => {
            ...
            // Test ready tasks - task2 should not be ready
            const readyTasks = await taskModel.getReadyTasks(workflow.id);
            expect(readyTasks.some(t => t.id === task1.id)).toBe(true);
            expect(readyTasks.some(t => t.id === task2.id)).toBe(false);

            // Complete task1
            await taskModel.updateStatus(task1.id, 'completed');

            // Now task2 should be ready
            const readyTasksAfter = await taskModel.getReadyTasks(workflow.id);
            expect(readyTasksAfter.some(t => t.id === task2.id)).toBe(true);
    ...
    describe('Error Handling', () => {
        test('should handle invalid workflow creation', async () => {
            await expect(workflowModel.create({
                // Missing required name field
                description: 'Invalid workflow'
            })).rejects.toThrow();
        });

        test('should handle invalid task creation', async () => {
        ...
        test('should handle non-existent record updates', async () => {
            const result = await taskModel.update('00000000-0000-0000-0000-000000000000', {
                title: 'Updated Title'
            });
        ...
        test('should handle non-existent record deletion', async () => {
            const result = await taskModel.delete('00000000-0000-0000-0000-000000000000');
