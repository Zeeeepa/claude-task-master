...
import { AICICDSystem, createAICICDSystem, processRequirement } from '../src/ai_cicd_system/index.js';
...
class FullTestAnalysis {
    ...
    async analyzeComponentExistence() {
        ...
        try {
            ...
            for (const componentName of expectedComponents) {
                const component = system.components.get(componentName.toLowerCase().replace(/([A-Z])/g, (match, letter) => 
    ...
    async testMockImplementations() {
        ...
        try {
            ...
            console.log(`✅ Advanced mock workflow completed: ${advancedResult.status}`);
    ...
    async testComponentIntegration() {
        ...
        try {
            ...
            if (components.requirementProcessor && components.taskStorage) {
                const analysisResult = await components.requirementProcessor.analyzeRequirement(
                ...
                const retrievedTask = await components.taskStorage.retrieveTaskById(taskId);
                console.log(`   ✅ Task stored and retrieved: ${retrievedTask.title}`);
                
                this.integrationTests.set('requirementToStorage', { success: true, taskId });
            }

            // Test 2: Task Storage → Context Manager → Codegen
            console.log('Testing: Task Storage → Context Manager → Codegen');
            if (components.taskStorage && components.contextManager && components.codegenIntegrator) {
                const tasks = await components.taskStorage.getPendingTasks();
                if (tasks.length > 0) {
                    const task = tasks[0];
                    const context = await components.contextManager.generatePromptContext(task.id);
                    const codegenResult = await components.codegenIntegrator.processTask(task, context);
                    
                    console.log(`   ✅ Codegen processing: ${codegenResult.status}`);
                    this.integrationTests.set('storageToCodegen', { success: true, result: codegenResult });
                }
            }
            ...
            if (components.validationEngine) {
                ...
                this.integrationTests.set('codegenToValidation', { success: true, result: validationResult });
            }
            ...
            if (components.workflowOrchestrator) {
                ...
                const orchestrationResult = await components.workflowOrchestrator.completeWorkflow(
                    'test-workflow',
                    workflowData
                );
                
                console.log(`   ✅ Workflow orchestration: ${orchestrationResult.status}`);
                this.integrationTests.set('workflowOrchestration', { success: true, result: orchestrationResult });
            }
            ...
            this.integrationTests.set('error', { success: false, error: error.message });
        }
    }
    ...
    async testRealAPIIntegration() {
        ...
        try {
            ...
            console.log('   This might be due to API limits, network issues, or credential problems');
    ...
    async testEndToEndWorkflows() {
        ...
        const testScenarios = [
            ...
            },
            {
                ...
                requirement: 'Implement JWT-based authentication with login, logout, and password reset'
            },
            {
                name: 'Database Integration',
                requirement: 'Add PostgreSQL database integration with user management and data validation'
            }
        ];
    ...
    async testPerformanceAndStress() {
        ...
        try {
            ...
            console.log(`   Successful: ${successful}/${concurrentRequirements.length}`);
            console.log(`   Failed: ${failed}/${concurrentRequirements.length}`);
            console.log(`   Average time per request: ${(duration / concurrentRequirements.length).toFixed(0)}ms`);
            ...
            for (let i = 0; i < 10; i++) {
                await system.processRequirement(`Test requirement ${i + 1}: Create a simple API endpoint`);
            }
            const loadTestDuration = Date.now() - loadTestStart;

            console.log(`✅ Load test completed in ${loadTestDuration}ms`);
            console.log(`   Average per request: ${(loadTestDuration / 10).toFixed(0)}ms`);
    ...
    async generateAnalysisReport() {
        ...
        if (this.realAPITests.size > 0 && !Array.from(this.realAPITests.values()).some(t => t.success)) {
    ...
    calculateSuccessRate(result) {
        ...
        if (result.codegen_results) {
            ...
            successful += result.codegen_results.filter(r => r.status === 'completed').length;
        }
        
        if (result.validation_results) {
            total += result.validation_results.length;
            successful += result.validation_results.filter(r => r.status === 'passed').length;
        }
    ...
    calculateOverallScore() {
        ...
        const componentScore = (Array.from(this.componentTests.values()).filter(t => t.exists).length / this.componentTests.size) * 40;
        score += componentScore;
        maxScore += 40;
        
        // Mock testing (20 points)
        const mockScore = (Array.from(this.mockTests.values()).filter(t => t.success).length / Math.max(this.mockTests.size, 1)) * 20;
        score += mockScore;
        maxScore += 20;
        
        // Integration testing (20 points)
        const integrationScore = (Array.from(this.integrationTests.values()).filter(t => t.success).length / Math.max(this.integrationTests.size, 1)) * 20;
        score += integrationScore;
        maxScore += 20;
        
        // E2E testing (20 points)
        const e2eScore = (this.testResults.filter(r => r.success).length / Math.max(this.testResults.length, 1)) * 20;
        score += e2eScore;
        maxScore += 20;
