...
async function testPerformance() {
    ...
    if (result.data.length !== 20 || result.pagination.total !== 50) {
        throw new Error('Pagination not working correctly');
    }
    
    // Test statistics
    const stats = await taskModel.getStatistics(workflow.id);
    if (stats.total_tasks !== 50) {
        throw new Error('Statistics calculation incorrect');
    }
...
async function validateDatabaseSchema() {
    log('🚀 Starting Database Schema Validation', 'bold');
    log('=' .repeat(50), 'blue');
    ...
    try {
        ...
        await runTest('Indexes', () => validateIndexes(db));
        await runTest('Triggers and Functions', () => validateTriggers(db));
        await runTest('Views', () => validateViews(db));
        ...
        await runTest('Performance Testing', testPerformance);
    ...
    if (results.failed === 0) {
        log('\n🎉 All validations passed! Database schema is ready for production.', 'green');
        process.exit(0);
    } else {
        log('\n⚠️  Some validations failed. Please review and fix the issues.', 'red');
        process.exit(1);
    }
}
